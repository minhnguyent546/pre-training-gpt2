...
def prepare_fineweb_edu(args: argparse.Namespace) -> None:
    ...
    print(
        f"Approximately {num_approx_shards} shards will be created with size {shard_size} tokens each"
    )
    ...
    with mp.Pool(num_workers) as pool:
        for item in pool.imap(partial(tokenize_example, tokenizer=tokenizer), ds, chunksize=128):
            ...
            if cur_num_tokens + token_count > shard_size:
                ...
                shard_idx += 1
                progress_bar = tqdm(
                    desc=f"Processing shard {shard_idx}", total=shard_size, unit=" tokens"
                )
                arr.fill(dump_id)
